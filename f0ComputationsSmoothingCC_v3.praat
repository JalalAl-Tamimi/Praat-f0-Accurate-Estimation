...
for i from 1 to numberOfFiles
	...
	Read from file: "'directory1$'/'name$'.TextGrid"
   	...
   	for j from 1 to nbIntervals
		...
		ceiling = q3*1.5
		selectObject: "Sound 'name$'"
		noprogress To Pitch (raw cross-correlation): 0.005, floor, ceiling, 15, "yes", 0.03, 0.45, 0.01, 0.35, 0.14
		Rename: "Speaker Adapted"
		Save as text file: directory1$ + "/PitchFiles" + "/" + "'name$'_Corrected_Non_Smoothed_CC" + ".Pitch"

		### smoothed pitch contours
		Smooth: 15
		Rename: "Smoothed 1"
		
		Save as text file: directory1$ + "/PitchFiles" + "/" + "'name$'_Corrected_Smoothed_CC" + ".Pitch"
